...
//        db.close();
        ...
        for(int i=0;i<dsSach.size();i++){
            dsTenSP.add(dsSach.get(i).getBookName());
            dsTenSP.add(String.valueOf(dsSach.get(i).getPage()));
            dsTenSP.add(dsSach.get(i).getDescription());
        }
        View viewcau4=inflater.inflate(R.layout.fragment_cau4, container, false);
        ListView listViewcau4 = viewcau4.findViewById(R.id.listviewcau4);
        ArrayAdapter<String> arraysach = new ArrayAdapter<String>(
                getContext(), android.R.layout.simple_list_item_1, dsTenSP);
        listViewcau4.setAdapter(arraysach);
        return viewcau4;
    }
}